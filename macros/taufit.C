...
//////////////////////////////////////////////////////////////////////////////
// Name:             taufit.C
// Purpose:          Fit given data with an exp function
...
//                   Int_t NofTaus             -- number of exp function to be fitted
//                   Double_t A0               -- a0
//                   Double_t Tau0             -- tau0
//                   Double_t A1               -- a1
//                   Double_t Tau1             -- tau1
...
//////////////////////////////////////////////////////////////////////////////
   ...
   for (Int_t i = 0; i < fitNofTaus; i ++) {
      fitval = fitval + par[2 * i] * exp(-x[0] * log(2) / par[2 * i + 1]);
   }
...
//
{
	...
	// set up fit function
	TF1 * f = new TF1("taufit", multi_exp, g->GetXaxis()->GetXmax(), g->GetXaxis()->GetXmin(), 2 * fitNofTaus);
	...
	if (fitNofTaus > 1) {
		tl->DrawLatex(x0, y0, Form("f(x) = #sum_{i=0}^{%d}a_{i} #upoint e^{ -x #upoint 6.9315 / #tau_{i}}", fitNofTaus - 1));
	} else {
		tl->DrawLatex(x0, y0, "f(x) = a_{0} #upoint e^{ -x #upoint 6.9315 / #tau_{0}}");
	}

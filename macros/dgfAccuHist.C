...
// Syntax:           .x dgfAccuHist.C(const Char_t * Module,
//                             Int_t AccuTime,
...
void dgfAccuHist(const Char_t * Module = "<TMrbDGF.LastSelected",
         Int_t AccuTime = 10,
         const Char_t * TimeScale = "secs",
         Int_t GoodChannels = 1,
         Int_t TriggerMask = 1,
         Char_t * TriggerPolarity = "neg",
...
//>>_________________________________________________(do not change this line)
{
	...
	if (dgf == NULL) {
		cerr	<< setred
				<< "dgfAccuHist.C: Can't locate DGF-4C (\"" << Module << "\") - not defined?"
				<< setblack << endl;
		return;
	}

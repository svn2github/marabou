...
class TUsrHit : public TObject {
	...
	public:
		...
		const Char_t * EventTime2Ascii(TString & TimeString);
		...
		inline UShort_t * CopyEventTime(UShort_t * EventTime) { 						// copy event time
			for (Int_t i = 0; i < 3; i++) fEventTime[i] = *EventTime++;
			return(fEventTime);
		};
		inline UShort_t * CopyEventTime(TUsrHit * Hit) {
			UShort_t * eventTime = Hit->GetEventTime();
			for (Int_t i = 0; i < 3; i++) fEventTime[i] = *eventTime++;
			return(fEventTime);
		};

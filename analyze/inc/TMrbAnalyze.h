...
class TUsrHit : public TObject {
	...
	public:
		...
		Int_t Compare(UShort_t * ChannelTime);
		...
		inline UShort_t * GetChannelTime() { return(fChannelTime); };		// get pointer to event time
		...
		inline void SetChannelTime(UShort_t * ChannelTime) { for (Int_t i = 0; i < 3; i++) fChannelTime[i] = *ChannelTime++; };
		inline void SetChannelTime(UShort_t BufferTimeHi, UShort_t EventTimeHi, UShort_t FastTrigTime) {
				fChannelTime[0] = BufferTimeHi;
				fChannelTime[1] = EventTimeHi;
				fChannelTime[2] = FastTrigTime;
		};
		...
		inline UShort_t * CopyChannelTime(UShort_t * ChannelTime) { 						// copy event time
			for (Int_t i = 0; i < 3; i++) fChannelTime[i] = *ChannelTime++;
			return(fChannelTime);
		};
		inline UShort_t * CopyChannelTime(TUsrHit * Hit) {
			UShort_t * channelTime = Hit->GetChannelTime();
			for (Int_t i = 0; i < 3; i++) fChannelTime[i] = *channelTime++;
			return(fChannelTime);
		};
	...
	protected:		
		...
		UShort_t fChannelTime[3];	// time stamp generated by dgf clock bus (48 bits unsigned!)
...
class TUsrHBX : public TObject {
	...
	public:
		TUsrHBX(TUsrHitBuffer * HitBuffer, Int_t * OffsetTable, Int_t TableLength, Int_t Window = 0);		// ctor
		TUsrHBX(TUsrHitBuffer * HitBuffer, TArrayI & OffsetTable, Int_t Window = 0);		// ctor
		...
		inline void SetOffsetTable(Int_t * OffsetTable, Int_t NofOffsets) { fOffset.Set(NofOffsets, OffsetTable); };
		inline void SetOffsetTable(TArrayI & OffsetTable) { fOffset.Set(OffsetTable.GetSize(), OffsetTable.GetArray()); };
		inline Int_t GetOffset(Int_t ModuleIndex) { return(fOffset[ModuleIndex]); };
	...
	protected:
		...
		TArrayI fOffset;						// time stamp offsets, one per module

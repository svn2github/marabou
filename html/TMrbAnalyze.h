...
class TUsrHitBuffer : public TObject {
	...
	public:
		...
		inline void ResetIndex() { fCurIndex = 0; };
		...
		TUsrHit * First() { return ((TUsrHit *) fHits->First()); }; 	// return next hit
		TUsrHit * After(TUsrHit * Hit) { return ((TUsrHit *) fHits->After(Hit)); }; 	// return next hit
		
		TUsrHit * FindEvent(Int_t EventNumber); 							// find an event
		TUsrHit * FindNextEvent();											// find first hit of next event

		TUsrHit * NextHitInWindow(UShort_t * Time0);						// get next hit
		inline TUsrHit * NextHitInWindow(TUsrHit * Hit0) { return(this->NextHitInWindow(Hit0->GetEventTime())); };
		inline TUsrHit * NextHit() { return((fCurIndex < fNofEntries) ? (TUsrHit *) fHits->At(fCurIndex) : NULL); };

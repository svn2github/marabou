...
{
   Double_t aa = TMath::Abs(alpha);
   Double_t cosa = TMath::Cos(aa);
   Double_t sina = TMath::Sin(aa);

   return a * cosa + b * sina;
}
//__________________________________________________________________________
...
{   
   ...
   Double_t cosa = TMath::Cos(aa);
   Double_t sina = TMath::Sin(aa);
   Double_t tana = TMath::Tan(aa);
   if (alpha > 0) {
      return cosa * (y - a * sina * (1 - x / (a * cosa)));
   } else {
      return y /cosa - tana * x_orig(a, b, alpha, x, y); 
   }
}
//__________________________________________________________________________
...
{   
   ...
   if (alpha > 0) {
      Double_t y2 = a * sina * (1 - x /(a * cosa));
      return a - (y - y2) * sina - y2 / sina;
   } else {
      return (x / cosa - b * tana  + y * tana / cosa) / ( 1 + tana * tana); 
   }
}
//__________________________________________________________________________
...
{
   Axis_t xr = hist->GetXaxis()->GetXmax() -  hist->GetXaxis()->GetXmin();
   Axis_t yr = hist->GetYaxis()->GetXmax() -  hist->GetYaxis()->GetXmin();
   Int_t binx =(Int_t) ((Axis_t) hist->GetNbinsX() * x / xr + 1.);
//   cout << binx << endl;
   if (binx < 1 || binx > hist->GetNbinsX()) return 0;
   Int_t biny =(Int_t) ((Axis_t) hist->GetNbinsY() * y / yr + 1.);
//   cout << biny << endl;
   if (biny < 1 || biny > hist->GetNbinsY()) return 0;
...
//__________________________________________________________________________
/*
TH2 * rotate_hist(TH2 * hist, Double_t alpha_deg, Int_t serial_nr)
{
   if (!hist) return NULL;
   Axis_t xl = hist->GetXaxis()->GetXmin();
   Axis_t xu = hist->GetXaxis()->GetXmax();
   Axis_t yl = hist->GetYaxis()->GetXmin();
   Axis_t yu = hist->GetYaxis()->GetXmax();
   Axis_t xr = xu - xl;
   Axis_t yr = yu - xl;
//  set axis limits starting at 0
   hist->GetXaxis()->Set(hist->GetYaxis()->GetNbins(),0,xr); 
   hist->GetYaxis()->Set(hist->GetYaxis()->GetNbins(),0,yr); 
   
   Axis_t xbw = hist->GetXaxis()->GetBinWidth(1);
   Axis_t ybw = hist->GetYaxis()->GetBinWidth(1);
   Double_t alpha = TMath::Pi() * alpha_deg / 180.;
   Axis_t xa = xa_new(xr, yr, alpha);
   Axis_t ya = ya_new(xr, yr, alpha);
   Int_t nbinx = (Int_t)(xa / xbw + 1.);
   Int_t nbiny = (Int_t)(ya / ybw + 1.);
   xa = nbinx * xbw;
   ya = nbiny * ybw;
   TString hname(hist->GetName());
   Int_t us = hname.Index("_");
...
//      cout << "rotate_hist,hold: " << hold << endl;
   ...
   for (Int_t binx = 1; binx <= nbinx; binx++) {
      for (Int_t biny = 1; biny <= nbiny; biny++) {
         Axis_t x = hist->GetXaxis()->GetBinCenter(binx);
         Axis_t y = hist->GetYaxis()->GetBinCenter(biny);
         Axis_t xo = x_orig(xr, yr, alpha, x, y);
         Axis_t yo = y_orig(xr, yr, alpha, x, y);
         hrot->SetBinContent(binx, biny, get_bincontent(hist, xo, yo));
      }
   }
//  reset axis limits starting at 0
   hist->GetXaxis()->Set(hist->GetYaxis()->GetNbins(), xl, xu); 
   hist->GetYaxis()->Set(hist->GetYaxis()->GetNbins(), yl, yu); 
   return hrot;
}
*/
//__________________________________________________________________________

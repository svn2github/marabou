...
class TMrbSubevent : public TNamed {
	...
	public:
		...
		inline void FillHistogramsIfPositive(Bool_t Flag = kTRUE) { fHistosToBeFilledIfPositive = Flag; };
		inline Bool_t HistosToBeFilledIfPositive() { return(fHistosToBeFilledIfPositive); };
	...
	protected:
		...
		Bool_t fHistosToBeFilledIfPositive;	// fill histograms if index is positive only

...
static FILE * _unix_open(const char * FileName);
static TUnixEntry * _unix_find_entry(const char * EntryName, int ValueNumber);
int _unix_find_value(TUnixEntry * Entry, int ValueNumber);
static void _unix_decode_entry(TUnixEntry * Entry, const char * UnixString);

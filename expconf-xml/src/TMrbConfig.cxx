...
TMrbConfig::TMrbConfig(const Char_t * CfgName, const Char_t * CfgTitle)
										: TNamed(CfgName, CfgTitle), TMrbXMLCodeClient() {
...
//////////////////////////////////////////////////////////////////////////////
	...
	if (gMrbConfig != NULL) {
		...
		TMrbXMLCodeClient::fVerboseMode = fVerboseMode;
...
>>>>>>> 1.155
		...
		fLofTags.SetName("Tags");								// ... tags (combined)
		fLofTags.AddNamedX(kMrbLofReadoutTags);	
		fLofTags.AddNamedX(kMrbLofAnalyzeTags);
		fLofTags.AddNamedX(kMrbLofConfigTags);
		fLofTags.AddNamedX(kMrbLofRcFileTags);
		fLofTags.AddNamedX(kMrbLofUserMacroTags);
		fLofTags.AddNamedX(kMrbLofUserEventTags);
		fLofTags.AddNamedX(kMrbLofXhitTags);
		fLofTags.AddNamedX(kMrbLofModuleTags);

		this->GetAuthor();		 								// author's name
<<<<<<< TMrbConfig.cxx
		this->GetMailAddr();		 							// author's mail addr
		this->GetURL();				 							// author's home page
=======
...
//////////////////////////////////////////////////////////////////////////////
	...
	TString srcPath = gEnv->GetValue("TMrbConfig.SrcPath", gSystem->WorkingDirectory());
	gSystem->ExpandPathName(srcPath);
	
	TString prefix = this->GetName();
	prefix(0,1).ToUpper();
	...
	packNames acxx(cfile.Data(), "Analyze.cxx.xml", ".cxx", "C++ code to be used with ROOT");
	filesToCreate.Add((TObject *) &acxx);

	packNames ah(cfile.Data(), "Analyze.h.xml", ".h", "C++ class definitions");
	filesToCreate.Add((TObject *) &ah);

	packNames aldf(cfile.Data(), "AnalyzeLinkDef.h.xml", "LinkDef.h", "CINT directives");
	filesToCreate.Add((TObject *) &aldf);

	packNames amk(cfile.Data(), "Analyze.mk.xml", ".mk", "Makefile (ROOT)");
	filesToCreate.Add((TObject *) &amk);

	packNames agl(cfile.Data(), "AnalyzeGlobals.h.xml", "Globals.h", "User's global pointers (ROOT)");
	filesToCreate.Add((TObject *) &agl);

	packNames aul(clibf.Data(), "LoadLibs.C.xml", "LoadLibs.C", "Macro to load libs for an interactive ROOT session");
	filesToCreate.Add((TObject *) &aul);

	packNames ci(cidxf.Data(), "CommonIndices.h.xml", "CommonIndices.h", "Common indices for analysis AND readout");
	filesToCreate.Add((TObject *) &ci);

	packNames aht(cfile.Data(), "Analyze.html.xml", ".html", "HTML documentation, class index");
	filesToCreate.Add((TObject *) &aht);
	...
	TIterator * ppIter = filesToCreate.MakeIterator();
	while (fPck = (packNames *) ppIter->Next()) {
		TString cf = fPck->GetF() + fPck->GetX();
		if (((this->GetAnalyzeOptions() & kAnaOptOverwrite) == 0) && (gSystem->AccessPathName(cf) == 0)) {
			gMrbLog->Err() << "File exists - " << cf << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode");
			continue;
		...
		TString fileSpec;
		TMrbSystem ux;
		ux.Which(fileSpec, xmlPath.Data(), tf.Data());
		if (fileSpec.IsNull()) {
			tf = fPck->GetT();
			tf2 = tf;
			ux.Which(fileSpec, xmlPath.Data(), tf.Data());
		...
		if (fileSpec.IsNull()) {
			gMrbLog->Err()	<< "XML file not found -" << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode");
			gMrbLog->Err()	<< "Searching on path " << xmlPath << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode", setred, kTRUE);
			gMrbLog->Err()	<< "                  for file " << tf1 << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode", setred, kTRUE);
			gMrbLog->Err()	<< "                  or       " << tf2 << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode", setred, kTRUE);
		...
		if (verboseMode) {
			gMrbLog->Out()  << "Using XML file " << fileSpec << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode");
		}
	
		TString xmlFile = fileSpec;

		TMrbXMLCodeGen * codeGen = new TMrbXMLCodeGen(xmlFile, this, verboseMode);
		if (codeGen->IsZombie()) {
			gMrbLog->Err() << "Can't process XML file - " << xmlFile << endl;
			gMrbLog->Flush("TMrbConfig", "MakeAnalyzeCode");
			return(kFALSE);
		}
		delete codeGen;
		codeGen = NULL;
		gMrbLog->Out() << "[" << cf << ": " << fPck->GetC() << "]" << endl;
		gMrbLog->Flush("", "", setblue);
	}
...
Bool_t TMrbConfig::MakeAnalyzeCode(TString & Code, const Char_t * ClassName, const Char_t * CodeFile,
																		TMrbNamedX * Tag, const Char_t * Extension) {
//________________________________________________________________[C++ METHOD]
//////////////////////////////////////////////////////////////////////////////
// Name:           TMrbConfig::MakeAnalyzeCode
// Purpose:        Generate class-specific code for user's analysis
// Arguments:      TString & Code              -- where to store code
//                 Char_t * ClassName          -- class name
//                 Char_t * CodeFile           -- common code file
//                 TMrbNamedX * Tag            -- tag to be searched for
//                 Char_t * Extension          -- file extension
// Results:        kTRUE/kFALSE
// Exceptions:
// Description:    Generates class-specific code.
...
//////////////////////////////////////////////////////////////////////////////
	...
	if (!this->TagToBeProcessed(tf.Data(), Tag->GetIndex())) { 	// will be processed only once
		if (verboseMode) {
			gMrbLog->Out()  << "[" << ClassName << "] XML file " << fileSpec
							<< ": Tag " << Tag->GetName()
							<< "(" << Tag->GetIndex() << ") already processed" << endl;
			gMrbLog->Flush(this->ClassName(), "MakeAnalyzeCode");
		}
		return(kTRUE);
	}

	this->AddToTagList(tf.Data(), Tag->GetIndex());
	
	TString xmlFile = fileSpec;

	TMrbXMLCodeGen * codeGen = new TMrbXMLCodeGen(xmlFile, this, Tag->GetName(), NULL, verboseMode);
	if (codeGen->IsZombie()) {
		gMrbLog->Err() << "Can't process XML file - " << xmlFile << endl;
		gMrbLog->Flush("TMrbConfig", "MakeAnalyzeCode");
		return(kFALSE);
...
//////////////////////////////////////////////////////////////////////////////
	...
	if (fileSpec.IsNull()) {
		...
		gMrbLog->Flush(this->ClassName(), "CallUserMacro", setred, kTRUE);
...
//////////////////////////////////////////////////////////////////////////////
	...
	while (fPck = (packNames *) ppIter->Next()) {
		...
		if (fileSpec.IsNull()) {
			...
			gMrbLog->Flush(this->ClassName(), "CreateXhit");
			gMrbLog->Err()	<< "Searching on path " << templatePath << endl;
			gMrbLog->Flush(this->ClassName(), "CreateXhit", setred, kTRUE);
			outStrm.close();
			continue;
		}
...
// Exceptions:
<<<<<<< TMrbConfig.cxx
// Description:    Reads the author's name either from shell
//                 or from ldap database.
=======
// Description:    Reads the author's name via python-ldap.
>>>>>>> 1.155
// Keywords:
...
<<<<<<< TMrbConfig.cxx
	...
	if (fAuthor.IsNull()) fAuthor = fUser;
	return(fAuthor.Data());
}
...
//////////////////////////////////////////////////////////////////////////////
// Name:           TMrbConfig::GetMailAddr
// Purpose:        Get author's mail address
// Arguments:      
// Results:        
// Exceptions:
// Description:    Reads the author's mail address either from .rootrc
//                 or from ldap database.
// Keywords:
//////////////////////////////////////////////////////////////////////////////

	fMailAddr = gEnv->GetValue("User.MailAddr", "");
	if (fMailAddr.IsNull()) {
...
const Char_t * TMrbConfig::GetURL() {
//________________________________________________________________[C++ METHOD]
//////////////////////////////////////////////////////////////////////////////
// Name:           TMrbConfig::GetURL
// Purpose:        Get author's home page
// Arguments:      
// Results:        
// Exceptions:
// Description:    Reads the author's home page from .rootrc
// Keywords:
//////////////////////////////////////////////////////////////////////////////

	fURL = gEnv->GetValue("User.URL", "??");
	return(fURL.Data());
=======
	return(fMailAddr.Data());
>>>>>>> 1.155

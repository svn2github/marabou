...
public:
   ...
   inline const Char_t * ClassName() const { return "TString"; };
};
...
extern char *Strip(const char *str);         // strip blanks of str, free with delete []
...
{ return Replace(Length(), 0, s.Data(), n <= s.Length() ? n : s.Length()); }
...
{ return Replace(pos, 0, s.Data(),  n <= s.Length() ? n : s.Length()); }
...
inline TString& TString::Replace(Ssiz_t pos, Ssiz_t n1, const TString& s,
                                 Ssiz_t n2)
{ return Replace(pos, n1, s.Data(), s.Length() <= n2 ? s.Length() : n2); }

...
void TList::AddBefore(const TObject *before, TObject *obj)
{
   ...
   if (!before)
      ...
      if (!t) {
         cerr << "AddBefore: before not found, object not added" << endl;
         return;
      }
      if (t == fFirst)
         TList::AddFirst(obj);
...
void TList::AddAfter(const TObject *after, TObject *obj)
{
   ...
   if (!after)
      ...
      if (!t) {
         cerr << "AddAfter: after not found, object not added" << endl;
         return;
      }
      if (t == fLast)
         TList::AddLast(obj);
...
void TList::Sort(Bool_t order)
{
   ...
   if (!fFirst->GetObject()->IsSortable()) {
      cerr << "Sort: objects in list are not sortable" << endl;
      return;
   }

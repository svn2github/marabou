...
//////////////////////////////////////////////////////////////////////////////
//! \file			caen_v1190.c
//! \brief			test program for Mesytec caen_v1190
//! \details		Provides some basic tests for caen_v1190
//!
...
static struct s_caen_v1190 * s_tdc;

void f_ut_send_msg(char * prefix, char * msg, int flag);
void read_micro_reg(struct s_caen_v1190 * s, char *argv[]);
void write_micro_reg(struct s_caen_v1190 * s, char *argv[]);
...
int main(int argc, char *argv[]) {
	...
	if (argc <= 2) {
		fprintf(stderr, "caen_v1190: Basic tests for CAEN V1190 tdc modules\n\n");
		fprintf(stderr, "Usage: caen_v1190 <addr> [<fct> [<args>]]\n\n");
	...
	s_param.iack = 1;						/* prepare vme segment */
 	...
 	caen_v1190 = (volatile char *) find_controller(physAddr, 0x10000L, addrMod, 0, 0, &s_param);
	if (caen_v1190 == (volatile char *) -1) {
		fprintf(stderr, "caen_v1190: Can't map addr %#lx (mod=%#lx)\n", physAddr, addrMod);
		exit(1);
	}

	s_tdc = caen_v1190_alloc(physAddr, caen_v1190, "caen_v1190", 0xBB);	/* allocate data struct */
	if (s_tdc == NULL) exit(1);

	caen_v1190_setPrefix(s_tdc, "caen_v1190");

	file[0] = '\0';
	if (argc >= 4) strcpy(file, argv[3]);

	if (strcmp(argv[2], "v") == 0) {
		caen_v1190_moduleInfo(s_tdc);				/* read module info */
	} else if (strcmp(argv[2], "r") == 0) {
		caen_v1190_reset(s_tdc);					/* reset module */
	} else if (strcmp(argv[2], "c") == 0) {
		caen_v1190_softClear(s_tdc);				/* clear module */
		...
		if (file[0] == '\0') {
			fprintf(stderr, "caen_v1190: File name missing\n");
			exit(1);
		}
		caen_v1190_fillStruct(s_tdc, file);			/* fill struct from environment data */
		caen_v1190_loadFromDb(s_tdc); 				/* load module regs from database */
		...
		caen_v1190_dumpRegisters(s_tdc, file); 		/* dump registers to file */
	} else if (strcmp(argv[2], "p") == 0) {
		s_tdc->dumpRegsOnInit = TRUE;
		caen_v1190_dumpRegisters(s_tdc, NULL); 		/* print registers */
		...
		fprintf(stderr, "caen_v1190: Illegal function - %s\n", argv[2]);
		exit(1);
	}
	exit(0);
}
...
void read_micro_reg(struct s_caen_v1190 * s, char *argv[]) {
	...
	if (strcmp(reg, "tm") == 0) {
		...
		fprintf(stderr, "caen_v1190: Wrong register name - %s\n", reg);
	}
}
...
void write_micro_reg(struct s_caen_v1190 * s, char *argv[]) {
	...
	if (strcmp(reg, "tm") == 0) {
		caen_v1190_setTriggerMatchingOn(s, (data == 0) ? FALSE : TRUE);
	} else if (strcmp(reg, "ww") == 0) {
		caen_v1190_setWindowWidth(s, data);
	} else if (strcmp(reg, "wo") == 0) {
		caen_v1190_setWindowOffset(s, data);
	} else if (strcmp(reg, "sm") == 0) {
		caen_v1190_setSearchMargin(s, data);
	} else if (strcmp(reg, "rm") == 0) {
		caen_v1190_setRejectMargin(s, data);
	} else if (strcmp(reg, "st") == 0) {
		caen_v1190_enableTriggerTimeSubtraction(s, (data == 0) ? FALSE : TRUE);
	} else if (strcmp(reg, "rs") == 0) {
		res = strtol(argv[5], NULL, 0);
		if (data == 0) {
			width = strtol(argv[6], NULL, 0);
			caen_v1190_setResolution(s, data, res, width);
		} else {
			caen_v1190_setResolution(s, data, res, NO_ARG);
		}
	} else if (strcmp(reg, "dt") == 0) {
		caen_v1190_setDeadTime(s, data);
	} else if (strcmp(reg, "es") == 0) {
		caen_v1190_setEventSize(s, data);
	} else if (strcmp(reg, "fs") == 0) {
		caen_v1190_setFifoSize(s, data);
	} else if (strcmp(reg, "ht") == 0) {
		caen_v1190_enableHeaderTrailer(s, (data == 0) ? FALSE : TRUE);
	} else if (strcmp(reg, "tt") == 0) {
		caen_v1190_enableExtendedTriggerTag(s, (data == 0) ? FALSE : TRUE);
	} else if (strcmp(reg, "af") == 0) {
		caen_v1190_setAlmostFullLevel(s, data);
	} else {
		fprintf(stderr, "caen_v1190: Wrong register name - %s\n", reg);
	}
}

...
int main(int argc, char *argv[]) {
	...
	if (argc <= 4) {
		fprintf(stderr, "caen785: Basic tests for CAEN V785 modules\n\n");
		fprintf(stderr, "Usage: caen785 <addr> <addrMod> <mapMod> [<fct> [<args>]]\n\n");
	...
	if (mappingMod == 2) smem_remove("caen785");		/* remove previous mappings */

	md = mapVME("caen785", physAddr, 0x10000L, addrMod, mappingMod, 0);
	if (md == NULL) {
		fprintf(stderr, "caen785: Can't map addr %#lx (mod=%#lx)\n", physAddr, addrMod);
		exit(1);
	}
	...
	switch (fct) {
		...
		default:	fprintf(stderr, "caen785: Illegal function - %s\n", argv[4]);
					exit(1);
	}
	exit(0);
}
...
void caen_list_thresh(volatile unsigned char * vmeAddr) {
	...
	for (chn = 0; chn < 32; chn++) {
		thresh = CAEN_V785_GET_THRESHOLD(vmeAddr, chn);

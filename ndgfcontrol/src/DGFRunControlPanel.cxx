...
// Revision:       $Id: DGFRunControlPanel.cxx,v 1.1 2009-09-23 10:46:24 Marabou Exp $       
// Date:           
// URL:            
// Keywords:       
// Layout:
...
Bool_t newTabCreated=kFALSE;
	...
	switch (Selection) {
		...
		case kDGFRunControlTabSystem:
			if (fSetupPanel == NULL) {fSetupPanel = new DGFSetupPanel(fSystemTab); newTabCreated=kTRUE;}
			break;

		case kDGFRunControlTabFiles:
			if (fSetFilesPanel == NULL) {fSetFilesPanel = new DGFSetFilesPanel(fFilesTab);newTabCreated=kTRUE;}
			break;

		case kDGFRunControlTabParams:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fParamsPanel == NULL) {fParamsPanel = new DGFParamsPanel(fParamsTab);newTabCreated=kTRUE;}
			} else {
		...
		case kDGFRunControlTabModules:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fInstrumentPanel == NULL) {fInstrumentPanel = new DGFInstrumentPanel(fModulesTab);newTabCreated=kTRUE;}
		...
		case kDGFRunControlTabTrace:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fTraceDisplayPanel == NULL) {fTraceDisplayPanel = new DGFTraceDisplayPanel(fTracesTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabUntrigTrace:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fUntrigTracePanel == NULL) {fUntrigTracePanel = new DGFUntrigTracePanel(fUntrigTracesTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabOffsets:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fOffsetsPanel == NULL){ fOffsetsPanel = new DGFOffsetsPanel(fOffsetsTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabMCA:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fMcaDisplayPanel == NULL) {fMcaDisplayPanel = new DGFMcaDisplayPanel(fMCATab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabTauFit1:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fTauDisplayPanel == NULL) {fTauDisplayPanel = new DGFTauDisplayPanel(fTauDisplayTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabTauFit2:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fTauFitPanel == NULL) {fTauFitPanel = new DGFTauFitPanel(fTauFitTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabSave:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fSaveModuleSettingsPanel == NULL) {fSaveModuleSettingsPanel = new DGFSaveModuleSettingsPanel(fSaveTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabRestore:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fRestoreModuleSettingsPanel == NULL){ fRestoreModuleSettingsPanel = new DGFRestoreModuleSettingsPanel(fRestoreTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabCopy:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fCopyModuleSettingsPanel == NULL) {fCopyModuleSettingsPanel = new DGFCopyModuleSettingsPanel(fCopyTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabMisc:
			if (gDGFControlData->IsOffline() || gDGFControlData->CheckIfStarted()) {
				if (fMiscPanel == NULL) {fMiscPanel = new DGFMiscPanel(fMiscTab);newTabCreated=kTRUE;}
			} else {
				new TGMsgBox(fClient->GetRoot(), this, "DGFControl: Error", "DGF module(s) not started", kMBIconStop);
			}
			break;
		...
		case kDGFRunControlTabCptm:
			if (fCptmPanel == NULL) {fCptmPanel = new DGFCptmPanel(fCptmTab);newTabCreated=kTRUE;}
...
// 	Resizes needed for proper layout... dont ask me why (without them it cuts of parts of some Buttons, e.g. in the MCA-Tab)
/*	if (newTabCreated==kTRUE){
		UInt_t x=GetWidth();
		UInt_t y=GetHeight();
// 		MapWindow();
	    	Resize(x-10,y);

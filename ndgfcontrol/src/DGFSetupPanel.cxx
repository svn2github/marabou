...
// Revision:       $Id: DGFSetupPanel.cxx,v 1.1 2009-09-23 10:46:24 Marabou Exp $       
// Date:           
// URL:            
// Keywords:       
// Layout:
...
////////////////////////////////////// Profiles Laden////////////////////////////////////////
	...
	stdProfile=gMrbLofProfiles->FindProfile("lightblue", kTRUE);
...
// DGF defs
	...
	fDGFFrame = new TNGMrbCheckButtonGroup(this, "DGF General", &fSetupDGFModes, -1,stdProfile,0, 1);		//geaendert!!!!!!!!!!
	HEAP(fDGFFrame);
	this->AddFrame(fDGFFrame,expandXLayout);
...
//	CAMAC defs
	...
	//HEAP(camacLayout);
	fCAMACFrame = new TGGroupFrame(this, "CAMAC", kVerticalFrame,  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->GC(),  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->Font(),  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->BG());
	HEAP(fCAMACFrame);
	this->AddFrame(fCAMACFrame,expandXLayout);
	...
	//HEAP(hEntryLayout);
	fCAMACHostEntry = new TNGMrbLabelCombo(fCAMACFrame, "CAMAC Host",
																NULL
																,stdProfile,-1,0,
																kLEWidth,
																kLEHeight,
	...
	Int_t numberLetters;
	for (numberLetters=0; ((TString)camacHost[numberLetters]).IsAlpha() && numberLetters<camacHost.Length();numberLetters++);
	fCAMACHostEntryStrings = new TMrbLofNamedX();
	for (Int_t id=0; id<DGFSetupPanel::kNofPPCs; id++){
		TString x = camacHost(0,numberLetters);
		x+="-";
		x+=id;
		fCAMACHostEntryStrings->AddNamedX(id,x);
		//fCAMACHostEntry->AddEntry(fCAMACHostEntryStrings->FindByIndex(id)->GetName(),id);

	}
	fCAMACHostEntry->AddEntry(fCAMACHostEntryStrings);	
	fCAMACHostEntry->GetComboBox()->Select(camacHost(numberLetters,camacHost.Length()).String().Atoi(),kFALSE);				
	...
	fCAMACFrame->AddFrame(fCAMACHostEntry,expandXLayout);
	//fCAMACFrame->AddFrame(fCAMACHostEntry/*, frameGC->LH()*/);
	//fCAMACHostEntry->AddToFocusList(&fFocusList);


// download codes
	fCodeFrame = new TGGroupFrame(this, "Download", kHorizontalFrame,  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->GC(),  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->Font(),  stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->BG());
	HEAP(fCodeFrame);
	this->AddFrame(fCodeFrame,expandXLayout);
	//this->AddFrame(fCodeFrame/*, groupGC->LH()*/);
	
	fCodes = new TNGMrbCheckButtonList(fCodeFrame, NULL, &fSetupDGFCodes, -1,stdProfile,1, 0,
											TabFrame->GetWidth(), kLEHeight,0, lofSpecialButtons, kTRUE);
	HEAP(fCodes);
	fCodeFrame->AddFrame(fCodes, expandXLayout);
...
// modules
	...
	HEAP(fModules);
	this->AddFrame(fModules, expandXLayout);
	...
	fSelectFrame = new TGHorizontalFrame(fModules, TabFrame->GetWidth(), TabFrame->GetHeight(), kChildFrame, stdProfile->GetGC(TNGMrbGContext::kGMrbGCGroupFrame)->BG());
	HEAP(fSelectFrame);
	fModules->AddFrame(fSelectFrame, expandXLayout);
	//fModules->AddFrame(fSelectFrame/*, frameGC->LH()*/);
	
	for (Int_t i = 0; i < kNofModulesPerCluster; i++) {
		fColSelect[i] = new TNGMrbPictureButtonList(fSelectFrame,  NULL, &colSelect[i], -1,stdProfile,1, 0, 
							TabFrame->GetWidth(), kLEHeight);
		HEAP(fColSelect[i]);
		fSelectFrame->AddFrame(fColSelect[i],expandXLayout);
		//fSelectFrame->AddFrame(fColSelect[i]/*, frameGC->LH()*/);
		((TNGMrbButtonFrame *) fColSelect[i])->Connect("ButtonPressed(Int_t, Int_t)", this->ClassName(), this, "SelectModule(Int_t, Int_t)");
	}
	fAllSelect = new TNGMrbPictureButtonList(fSelectFrame,  NULL, &allSelect, -1,stdProfile,1, 0, 
							TabFrame->GetWidth(), kLEHeight);
	HEAP(fAllSelect);
	fSelectFrame->AddFrame(fAllSelect, new TGLayoutHints(kLHintsCenterY, 	hFrameLayout->GetPadLeft(),
...
// Connect buttons
	fActionFrame = new TNGMrbTextButtonGroup(this, "Actions", &fSetupConnect, -1,stdProfile,1, 0);
	HEAP(fActionFrame);
	this->AddFrame(fActionFrame, expandXLayout);
...
//	TabFrame->ChangeBackground(gDGFControlData->fColorBlue);
	...
	TGLayoutHints * dgfFrameLayout = new TGLayoutHints(kLHintsTop | kLHintsLeft | kLHintsExpandX, 5, 1, 5, 1);
	HEAP(dgfFrameLayout);
...
//////////////////////////////////////////////////////////////////////////////
	...
	TString camacHost = fCAMACHostEntryStrings->FindByIndex(fCAMACHostEntry->GetComboBox()->GetSelected())->GetName();
	gDGFControlData->fCAMACHost = camacHost;

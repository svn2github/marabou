...
// Revision:       $Id: DGFControlData.cxx,v 1.2 2010-12-15 09:07:47 Marabou Exp $       
// Date:           
// URL:            
// Keywords:       
//////////////////////////////////////////////////////////////////////////////
...
// open DGF specific resource data base
	TString rcFile;
	fRootrc->Get(rcFile, ".RcFile", ".DGFControl.rc");
	...
	if (ok) {
		...
		TString path;
		fRootrc->Get(path, ".DSPCode", "dsp/DGFcodeE.bin");
		fDSPCodeFile += path;
		...
		fRootrc->Get(fDgfSettingsPath, ".SettingsPath", "../dgfSettings");
		...
		path = fRootrc->Get(path, ".CptmSettingsPath", "");
...
//////////////////////////////////////////////////////////////////////////////
	...
	for (Int_t i = 0; i < nofSevts; i++) {
		TString sevtName;
		fDgfrc->Get(sevtName, ".Subevent", Form("%d", i), "Name", "");
		if (!sevtName.IsNull()) {
			TString sn = sevtName;
			sn(0,1).ToUpper();
			TString sevtNuna = Form("%d:%s", i, sn.Data());
			TString sevtClass;
			fDgfrc->Get(sevtClass, ".Subevent", sevtNuna.Data(), "ClassName", "");
			if (sevtClass.Index("TMrbSubevent_DGF", 0) == 0) {
				fNofClusters++;
				TString sevtTitle = "CLU";
				TString cs;
				sevtTitle += fDgfrc->Get(cs, ".Subevent", sevtNuna.Data(), "ClusterSerial", 0);
				sevtTitle += " ";
				sevtTitle += fDgfrc->Get(cs, ".Subevent", sevtNuna.Data(), "ClusterColor", "");
				...
				if (nofModules > 0) {
					...
					fDgfrc->Get(sevtLofModules, ".Subevent", sevtNuna.Data(), "LofModules", "");
					Int_t modNo = 0;
					TString dgfName;
					Int_t crate;
					Int_t from = 0;
					while (sevtLofModules.Tokenize(dgfName, from, ":")) {
						...
						if (!dgfModule->IsZombie())	{
							...
							TString addr;
							dgfName += Form(" (%s)", fDgfrc->Get(addr, ".Module", dgfNuna.Data(), "Address", ""));
